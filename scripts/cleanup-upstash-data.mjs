...
async function main() {
  ...
  // Load cleanup plan
  console.log('📋 Loading cleanup plan...');
  ...
  try {
    ...
    console.error('❌ Could not load cleanup plan:', error.message);
  ...
  const safetyFilter = (key) => {
    ...
    if (isProtected) {
      console.warn(`⚠️  SAFETY: Skipping protected key: ${key}`);
      return false;
    }
    return true;
  };
  ...
  console.log(`  ✓ Malformed keys: ${safeMalformed.length}`);
  console.log(`  ✓ Hanging references: ${safeHanging.length}`);
  console.log(`  ✓ Stale keys: ${safeStale.length}`);
  if (skippedCount > 0) {
    console.log(`  🛡️  Protected keys skipped: ${skippedCount} (likes/bookmarks)`);
  }
  console.log('');

  if (immediateCleanup.length === 0) {
    console.log('✨ No keys to cleanup - your database is clean!\n');
    return;
  }
  ...
  console.log('⚠️  STALE KEYS NOT INCLUDED:');
  ...
  console.log(`  ✅ These counts will NOT change after cleanup\n`);
  ...
  console.log('-'.repeat(80));
  console.log('✅ Run: npm run redis:analyze to verify cleanup');
  console.log('✅ Test your application to ensure everything works');
  console.log('✅ Monitor for any errors in production\n');
}
