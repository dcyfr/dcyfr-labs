  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (frontmatterMatch) {
        ...
        activePosts.push(frontmatter.slug || entry.name);
      }
    } catch (_err) {
...
async function checkPostCoverage(activePosts) {
  ...
  for (const slug of activePosts) {
    const hasCounter = await redis.exists(`views:post:${slug}`);
    const hasHistory = await redis.exists(`views:history:post:${slug}`);
    if (hasCounter) withCounters++;
    if (hasHistory) withHistory++;
    if (!hasCounter && !hasHistory) missingCounters.push(slug);
  }
  ...
  return { withCounters, withHistory, missingCounters };
}

async function verifyCounterSync(counterKeys) {
  ...
  for (const counterKey of counterKeys) {
    ...
    const historyCount = (await redis.zcard(`views:history:post:${slug}`)) || 0;
  ...
  return { syncedCount, desyncedCount, desynced };
}

async function checkOrphanedKeys(allCounterKeys) {
  ...
  const idBasedHistory = historyKeys.filter(
    (k) =>
      k.startsWith('views:history:post:post-') || k.startsWith('views:history:post:project-')
  );
  ...
  return { idBasedCounters, idBasedHistory, historyKeys };
}

async function countTotalViews(counterKeys, historyKeys) {
...
function printVerificationSummary({ desyncedCount, idBasedCounters, idBasedHistory, withCounters }) {
  console.log('\n' + '='.repeat(60));
  ...
  if (allGood) {
    ...
    if (idBasedHistory.length > 0) console.log(`      - ${idBasedHistory.length} orphaned history keys`);
...
async function main() {
  ...
  const counterKeys = allCounterKeys.filter(
    (k) => !k.includes(':day:') && !k.includes(':history:')
  );

  const { desyncedCount } = await verifyCounterSync(counterKeys);
  const { idBasedCounters, idBasedHistory, historyKeys } = await checkOrphanedKeys(allCounterKeys);
  await countTotalViews(counterKeys, historyKeys);

  printVerificationSummary({ desyncedCount, idBasedCounters, idBasedHistory, withCounters });
}
