...
function filterPhoneMatches(matches) {
  return matches.filter((m) => {
    ...
    if (/^(1234567890|0123456789|0000000000|1111111111|2222222222|9999999999)$/.test(digits))
      return false;
    return true;
  });
}
...
function filterBusinessIdMatches(matches, file, isContentFile, content, allowlist) {
  return matches.filter((m) => {
    if (/EXAMPLE|PLACEHOLDER|REDACTED|FAKE|MOCK|TEST|DUMMY|example\.com|placeholder/i.test(m))
      return false;
...
function scanContent(file, content, results, isPrScan, isContentFile) {
  ...
  for (const t of tests) {
    ...
    const isPiAllowed =
      isInAllowlist(file, allowlist.piPaths) || isInAllowlist(file, allowlist.proprietaryPaths);
    if (
      classification === 'PII' &&
      isPiiAllowed &&
      ...
      continue;
    if (classification === 'PI' && isPiAllowed && !['private_key', 'aws_key'].includes(t.name))
      continue;
