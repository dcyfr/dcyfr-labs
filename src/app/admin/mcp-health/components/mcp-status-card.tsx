...
import { CheckCircle2, AlertCircle, XCircle, Clock, Activity, Shield } from 'lucide-react';
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {sortedServers.map((server) => (
        <McpStatusCard key={server.name} server={server} metrics={metrics[server.name]} />
      ))}
    </div>
  );
}
