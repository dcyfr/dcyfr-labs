...
function findExportsInDir(dirPath) {
  ...
  try {
    const files = readdirSync(join(ROOT, dirPath)).filter(
...
function addExportToBarrel(barrelPath, exportName, sourceFile) {
  ...
  const existingExportLine = content
    .split('\n')
    .find((line) => line.includes(`from "./${sourceFile}"`) && line.includes('export {'));

  if (existingExportLine) {
    // Add to existing export
    const oldExport = existingExportLine;
    const exportList = oldExport.match(/{([^}]+)}/)[1];
    const newExport = oldExport.replace(exportList, `${exportList.trim()}, ${exportName}`);
...
function checkImport(name, source, filePath, missingExports) {
  ...
  if (!BARREL_DIRS.has(barrelPath)) return;
...
function buildMissingExportsMap() {
  ...
  for (const filePath of filesToScan) {
    const content = readFileSync(filePath, 'utf-8');
    for (const { name, source } of extractImports(content)) {
...
function reportSourceGroup(barrelPath, sourceFile, exps) {
  ...
  for (const exp of exps) {
    const additionalUsageCount = exp.files.length - 1;
    const additionalUsageLabel =
      additionalUsageCount > 0 ? ' (+' + additionalUsageCount + ' more)' : '';
    console.log(`    - ${exp.name}`);
    console.log(`      Used in: ${exp.files[0]}${additionalUsageLabel}`);
    count++;
  }
  if (AUTO_FIX) {
