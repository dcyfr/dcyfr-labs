...
export const securityAdvisoryMonitor = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    // Step 2: Check for new advisories (not previously seen)
    const newAdvisories = await step.run("filter-new-advisories", async () => {
      ...
      return advisories.filter((adv) => new Date(adv.publishedAt) >= oneDayAgo);
    }) as SecurityAdvisory[];
...
${newAdvisories.length} new security advisor${newAdvisories.length === 1 ? "y" : "ies"} detected:
