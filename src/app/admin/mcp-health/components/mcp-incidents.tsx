    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-xs">Issue history from the past 7 days</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <CardContent>
        <div className="space-y-4">
          {incidents.map((incident, index) => (
            <div key={`${incident.server}-${incident.timestamp}-${index}`} className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-1 truncate">{incident.error}</p>
                <div className="flex items-center gap-1 text-xs text-muted-foreground mt-1">
                  <Clock className="h-3 w-3" />
                  <span>{formatRelativeTime(incident.timestamp)}</span>
                  {incident.duration && <span className="ml-2">Duration: {incident.duration}</span>}
                </div>
              </div>
            </div>
          ))}
