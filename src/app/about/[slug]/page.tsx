...
export default async function ProfilePage({ params }: PageProps) {
  ...
  return (
    <PageLayout>
      ...
      {otherMember && (
        <div className={`mx-auto ${CONTAINER_WIDTHS.standard} ${CONTAINER_PADDING} py-12`}>
          <div className={SPACING.section}>
            <h2 className={TYPOGRAPHY.h2.standard}>Meet the Team</h2>
            <Link href={otherMember.profileUrl} className="block group mt-4">
              <Card
                className={`p-4 sm:p-8 ${SHADOWS.card.rest} ${SHADOWS.card.hover} ${HOVER_EFFECTS.cardLift}`}
              >
                <div className="flex items-start gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className={TYPOGRAPHY.h3.standard}>{otherMember.name}</h3>
                    ...
                    <p className="text-sm text-muted-foreground mb-3">{otherMember.title}</p>
