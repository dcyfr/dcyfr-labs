...
test.describe('Reading Progress Tracking', () => {
  ...
  test('should show progress bar on blog post', async ({ page }) => {
    ...
    // Progress bar should be visible (select first if multiple exist)
    const progressBar = page.locator('[role="progressbar"]').first();
    await expect(progressBar).toBeVisible();
    await expect(progressBar).toHaveAttribute('aria-valuenow', /\d+/, { timeout: 2000 });
  ...
  test('should update progress as user scrolls', async ({ page }) => {
    ...
    // Ensure page has scrollable content; if not, inject a test-only filler so progress can update
    ...
    // Wait for progress update (poll for aria-valuenow change)
    await expect(progressBar).toHaveAttribute('aria-valuenow', /\d+/, { timeout: 2000 });
  ...
  test('should persist progress to localStorage', async ({ page }) => {
    ...
    // Ensure page has scrollable content; inject test-only filler if needed
    const scrollable = await page.evaluate(
      () => document.documentElement.scrollHeight - window.innerHeight
    );
    if (scrollable < 200) {
    ...
    // Wait for progress to be saved (poll until reading-progress exists)
    await page.waitForFunction(() => !!localStorage.getItem('reading-progress'), { timeout: 2000 });
  ...
  test('should handle multiple articles in progress', async ({ page }) => {
    ...
    const articles = await page.goto('/blog').then(() => page.locator('article a').all());
