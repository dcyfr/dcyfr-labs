...
import { setMobileViewport, getViewportCategory } from './utils/mobile-viewports';
...
test.describe('Mobile Responsive Design', () => {
      ...
      if (await postList.isVisible()) {
        const columns = await postList.evaluate((el) => {
          ...
          const positions = children.map(
            (child) => (child as HTMLElement).getBoundingClientRect().left
          );
  ...
  test('text content width is readable on mobile (not stretched edge-to-edge)', async ({
    page,
    browserName,
  }) => {
    test.skip(!['webkit', 'chromium'].includes(browserName), 'Mobile browsers only');
  ...
  test('mobile layouts do not have elements that require horizontal scroll', async ({
    page,
    browserName,
  }) => {
    test.skip(!['webkit', 'chromium'].includes(browserName), 'Mobile browsers only');
  ...
  test('containers have max-width constraints to prevent text being too wide', async ({
    page,
    browserName,
  }) => {
    test.skip(!['webkit', 'chromium'].includes(browserName), 'Mobile browsers only');
  ...
  test('CSS media queries are respected (e.g., hidden/shown elements)', async ({
    page,
    browserName,
  }) => {
    ...
    const hiddenOnMobile = await page.evaluate(() => {
      ...
      const elements = document.querySelectorAll(
        '[class*="md:"], [class*="lg:"], [class*="hidden"], [class*="block"]'
      );
