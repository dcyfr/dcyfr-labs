...
import { Download, FileJson, FileSpreadsheet, FileText, Settings } from "lucide-react";
...
export function AnalyticsExport({ posts, summary, dateRange, filters }: AnalyticsExportProps) {
    ...
    posts.forEach(post => {
      ...
      columnMap.forEach(col => {
        if (col === 'title') {
          ...
          const rate = post.views > 0 ? ((post.shares + post.comments) / post.views * 100).toFixed(2) : '0.00';
          row.push(rate);
  ...
  const exportJSON = () => {
    const data: any = {
      posts: posts.map(post => {
        ...
        if (options.columns.engagement) {
          exportPost.engagementRate = post.views > 0 
            ? Number(((post.shares + post.comments) / post.views * 100).toFixed(2))
            : 0;
        }
        if (options.columns.publishedAt) exportPost.publishedAt = post.publishedAt;
    ...
    if (options.includeFilters && (filters.searchQuery || filters.selectedTags?.length)) {
  ...
  const exportMarkdown = () => {
    ...
    posts.forEach(post => {
      ...
      if (options.columns.engagement) {
        const rate = post.views > 0 ? ((post.shares + post.comments) / post.views * 100).toFixed(1) : '0.0';
        row.push(`${rate}%`);
      }
      if (options.columns.publishedAt) {
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56">
        ...
        <DropdownMenuCheckboxItem
          ...
          onCheckedChange={() => setOptions(prev => ({ ...prev, includeMetadata: !prev.includeMetadata }))}
        >
          Include Metadata
        </DropdownMenuCheckboxItem>
        <DropdownMenuCheckboxItem
          checked={options.includeSummary}
          onCheckedChange={() => setOptions(prev => ({ ...prev, includeSummary: !prev.includeSummary }))}
        >
          Include Summary
        </DropdownMenuCheckboxItem>
        ...
        <div className="max-h-48 overflow-y-auto">
          <DropdownMenuCheckboxItem checked={options.columns.title} onCheckedChange={() => toggleColumn('title')}>
            Title
          </DropdownMenuCheckboxItem>
          <DropdownMenuCheckboxItem checked={options.columns.views} onCheckedChange={() => toggleColumn('views')}>
            Views
          </DropdownMenuCheckboxItem>
          <DropdownMenuCheckboxItem checked={options.columns.shares} onCheckedChange={() => toggleColumn('shares')}>
            Shares
          </DropdownMenuCheckboxItem>
          <DropdownMenuCheckboxItem checked={options.columns.comments} onCheckedChange={() => toggleColumn('comments')}>
            Comments
          </DropdownMenuCheckboxItem>
          <DropdownMenuCheckboxItem checked={options.columns.engagement} onCheckedChange={() => toggleColumn('engagement')}>
            Engagement Rate
          </DropdownMenuCheckboxItem>
          <DropdownMenuCheckboxItem checked={options.columns.publishedAt} onCheckedChange={() => toggleColumn('publishedAt')}>
            Published Date
          </DropdownMenuCheckboxItem>
          <DropdownMenuCheckboxItem checked={options.columns.tags} onCheckedChange={() => toggleColumn('tags')}>
            Tags
          </DropdownMenuCheckboxItem>
        </div>
      </DropdownMenuContent>
