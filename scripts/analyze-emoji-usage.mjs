...
function printPublicContentSection() {
  if (results.publicContent.length === 0) return;
  ...
  const grouped = groupByFile(results.publicContent);
...
function printUIComponentsSection() {
  if (results.uiComponents.length === 0) return;
  console.log('\n⚠️  UI COMPONENTS (Review Needed)\n');
  console.log('These emojis appear in component code. Review if they are rendered to users.\n');
  const grouped = groupByFile(results.uiComponents);
  Object.entries(grouped).forEach(([file, items]) => {
    console.log(`  📄 ${path.relative(ROOT_DIR, file)}`);
...
function printResults() {
  ...
  printPublicContentSection();
  printUIComponentsSection();
