...
type MdxSectionProps = React.HTMLAttributes<HTMLElement> & {
  'data-footnotes'?: boolean;
  'data-mdx-footnotes-wrapper'?: string;
};
  ...
  return (tree: MdxTreeNode) => {
    const visit = (node: MdxTreeNode) => {
      if (
        ...
        const isFootnoteTextNode = (textNode: MdxTreeNode) =>
          textNode.type === 'text' && textNode.value === 'Footnotes';
        if (node.children && Array.isArray(node.children)) {
          node.children = node.children.filter((child: MdxTreeNode) => {
      ...
      if (node.type === 'element' && node.tagName === 'a') {
        ...
        if (node.properties?.href?.toString().includes('user-content-fnref')) {
          ...
          const hasEmoji = node.children?.some(
            (child: MdxTreeNode) => child.type === 'text' && child.value?.includes('↩')
          );

          if (hasEmoji) {
            // Replace text children containing ↩ with a marker we can target
            node.children = node.children?.map((child: MdxTreeNode) =>
...
function processPreCodeNode(node: MdxTreeNode): void {
  if (node.type !== 'element' || node.tagName !== 'pre') return;

  const codeChild = node.children?.find(
    (child: MdxTreeNode) => child.type === 'element' && child.tagName === 'code'
  );
...
const components: NonNullable<MDXRemoteProps['components']> = {
      ...
      if (React.isValidElement(child) && child.type === 'li') {
        ...
        return liChildren.some(
          (liChild) =>
            ...
            liChild.type === 'input' &&
            (liChild.props as Record<string, unknown>)?.type === 'checkbox'
        );
      }
      return false;
    });
  ...
  pre: (props: React.HTMLAttributes<HTMLPreElement>) => {
    ...
    let language = codeChild?.props
      ? ((codeChild.props as Record<string, unknown>)['data-language'] as string | undefined)
      : undefined;
  ...
  MCPArchitecture: MDXMCPArchitecture as React.ComponentType<Record<string, unknown>>,
  AuthenticationFlow: MDXAuthenticationFlow as React.ComponentType<Record<string, unknown>>,
  PipelineFlow: MDXPipelineFlow as React.ComponentType<Record<string, unknown>>,
  CVEDecisionTree: MDXCVEDecisionTree as React.ComponentType<Record<string, unknown>>,
  // FAQ component
  FAQ,
  ...
  section: (props: React.HTMLAttributes<HTMLElement>) => {
    const sectionProps = props as MdxSectionProps;
    const hasDataFootnotes =
      sectionProps['data-footnotes'] === true ||
      sectionProps['data-mdx-footnotes-wrapper'] === 'true';
