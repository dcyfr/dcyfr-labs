...
import { EmbedGenerator } from "@/components/activity/EmbedGenerator";
...
describe("EmbedGenerator", () => {
  ...
  it("renders with default options", () => {
    ...
    expect(screen.getByLabelText("Filter by Source (optional)")).toBeInTheDocument();
  ...
  it("copies embed code to clipboard", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByLabelText("Copy embed code")).toHaveAttribute("class", expect.stringContaining("green"));
    });
  });
  ...
  it("displays preview link with correct URL", () => {
    ...
    expect(previewLink).toHaveAttribute("href", expect.stringContaining("/activity/embed?source=github"));
  });
  ...
  it("combines multiple filter parameters correctly", () => {
    ...
    const codeBlock = screen.getByText(/source=blog.*timeRange=month.*limit=30/);
    expect(codeBlock).toBeInTheDocument();
  });
  ...
  it("displays usage instructions", () => {
    ...
    expect(screen.getByText(/Copy the generated embed code/)).toBeInTheDocument();
  });
