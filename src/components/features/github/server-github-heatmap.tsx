...
export async function ServerGitHubHeatmap({
  ...
  try {
    ...
    return (
      <Card className={`p-4 ${SEMANTIC_COLORS.alert.critical.container} ${SEMANTIC_COLORS.alert.critical.border}`}>
        <div className="flex items-start gap-3">
          <AlertCircle className={`w-5 h-5 mt-0.5 flex-shrink-0 ${SEMANTIC_COLORS.alert.critical.icon}`} />
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-sm ${SEMANTIC_COLORS.alert.critical.text}`}>
              We&apos;re unable to load your GitHub contribution data at this time. Please try again later.
            </p>
          </div>
        </div>
      </Card>
  ...
  if (data.contributions.length === 0 && data.source === 'error') {
    return (
      <Card className={`p-4 ${SEMANTIC_COLORS.alert.warning.container} ${SEMANTIC_COLORS.alert.warning.border}`}>
        <div className="flex items-start gap-3">
          <AlertCircle className={`w-5 h-5 mt-0.5 flex-shrink-0 ${SEMANTIC_COLORS.alert.warning.icon}`} />
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-sm ${SEMANTIC_COLORS.alert.warning.text}`}>
              {data.warning || 'Unable to retrieve your GitHub contribution history. Please check back later.'}
            </p>
          </div>
        </div>
      </Card>
