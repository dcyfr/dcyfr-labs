...
function loadReport(file) {
  try {
    ...
    console.error('ERROR: Could not read or parse report:', error.message);
    process.exit(2);
  }
}
...
function shouldBlock(match) {
  const combined =
    `${match.rule || ''} ${match.description || ''} ${match.tags || ''}`.toUpperCase();
...
function main() {
  ...
  const critical = report.filter((f) => {
    if (!shouldBlock(f)) return false;
    // If file is under a privateKeyPaths allowlist and has a reason that includes 'placeholder' or 'example', skip blocking
    const matchedPrivateKeyPath = privateKeyPaths.find((p) =>
      f.file?.startsWith(p.replace('/**', ''))
    );
    if (matchedPrivateKeyPath) {
      const reasons = allowlist.allowlistReasons || {};
      const reason = reasons[matchedPrivateKeyPath] || '';
