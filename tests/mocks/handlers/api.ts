 ...
 */
export const viewsApiHandler = http.post(
  `${BASE_URL}/api/views`,
  async (req) => {
    try {
      const body = (await req.request.json().catch(() => ({}))) as {
        postId?: string;
      };
      ...
      return HttpResponse.json(
        {
          ...
          timestamp: Date.now(),
          cached: false,
          postId: (body && body.postId) || "unknown",
        },
        { status: 200 }
      );
    } catch {
 ...
 */
export const analyticsApiHandler = http.get(
