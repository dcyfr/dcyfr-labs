 ...
 */
const schwarzschildRadius = (mass: number, scale: number = 0.025): number =>
  mass * scale;
 ...
 */
const iscoRadius = (rs: number, spin: number): number =>
  rs * (3 - 2.5 * Math.min(spin, 0.99));

/**
 * Photon Sphere Radius: Rph = 1.5Rs (Schwarzschild)
 * For Kerr: varies from 1-2Rs based on spin and orbit direction
 */
const photonSphereRadius = (rs: number, spin: number): number =>
  rs * (1.5 - 0.5 * spin);
 ...
 */
const ergosphereRadius = (rs: number, spin: number): number =>
  rs * (1 + Math.sqrt(1 - spin * spin * 0.5));
...
function BlackHole({
  ...
  const visuals = useMemo(
    () => computeBlackHoleVisuals(node, scaleFactor),
    [node, scaleFactor]
  );
  ...
  useFrame(() => {
    if (meshRef.current) {
      ...
      meshRef.current.rotation.y =
        elapsedTime * angularVelocity * (1 + node.spin * 0.5);
    }
  });
    ...
    if (pointsRef.current && particleGeometry) {
      ...
      nodes.forEach((node, nodeIdx) => {
        ...
        for (let i = 0; i < PHYSICS.particles.countPerNode; i++) {
          ...
          const keplerianOmega =
            baseOmega * Math.pow(visuals.iscoSize / orbitRadius, 1.5);
  ...
  const blackHoles: BlackHoleNode[] = useMemo(
    () => [
      ...
      // Ultramassive black hole powering central cluster galaxy
      {
        ...
        color: themeColor,
        mass: 14.4, // ~100 billion solar masses (scaled down 20%)
      ...
      // Extreme accretion, powerful relativistic jets
      {
        ...
        color: themeColor,
        mass: 11.5, // ~40-66 billion solar masses (scaled down 33%)
      ...
      // One of largest directly measured, unusually large for its galaxy
      {
        ...
        color: themeColor,
        mass: 8.0, // ~40 billion solar masses (scaled down 20%)
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      ...
      groupRef.current.rotation.x =
        Math.sin(elapsed * PHYSICS.groupRotation.xFrequency) *
        PHYSICS.groupRotation.xAmplitude;
    }
    ...
    const newPositions = blackHoles.map((node, idx) => {
      if (idx === 0) {
        ...
        const orbitAngle =
          (elapsed / PHYSICS.centralOrbit.period) * Math.PI * 2;
        return new THREE.Vector3(
          node.basePosition[0] +
            Math.cos(orbitAngle) * PHYSICS.centralOrbit.radiusX,
          node.basePosition[1] +
            Math.sin(orbitAngle * 0.7) * PHYSICS.centralOrbit.radiusY,
          node.basePosition[2] +
            Math.sin(orbitAngle * 1.3) * PHYSICS.centralOrbit.tilt
        );
      }
...
export function NetworkBackground() {
  ...
  return (
    ...
    >
      <Suspense
        fallback={
          <div className="absolute inset-0 bg-linear-to-br from-foreground/5 via-foreground/3 to-foreground/5" />
        }
      >
        <Canvas
