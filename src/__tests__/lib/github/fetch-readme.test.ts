...
import { readReadmeCache, writeReadmeCache } from "@/lib/github/cache";
...
describe("fetchRepoReadme", () => {
  ...
  it("handles base64 with newlines (GitHub API line-wraps at 60 chars)", async () => {
    ...
    stubFetch(async () =>
      mockFetchOk({ content: wrapped, encoding: "base64", name: "README.md", path: "README.md", size: 200, sha: "x", download_url: null })
    );
