...
export const metadata: Metadata = createPageMetadata({
  ...
  description:
    "Subscribe to RSS/Atom feeds for blog posts, projects, activity updates, and more. Stay up to date with the latest content.",
  path: "/feeds",
});
...
const feeds: FeedInfo[] = [
  ...
  },
  {
    ...
    description:
      "Combined blog posts and projects feed. Redirects to Activity Feed (kept for backward compatibility).",
    url: "/feed",
    updateFrequency: "Redirects to Activity Feed",
    icon: Rss,
    formats: [{ label: "Atom", url: "/feed", type: "application/atom+xml" }],
  },
];
        ...
        <div className={SPACING.section}>
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-4">
              Feeds allow you to subscribe to content updates without visiting
              the website. Use a feed reader like{" "}
              <a
          ...
          <div className="grid gap-4">
            {feeds
              .filter((feed) => feed.id !== "unified")
              .map((feed) => {
                const Icon = feed.icon;
                return (
                  <div
                    key={feed.id}
                    className="border rounded-lg p-4 hover:border-primary/50 transition-theme"
                  >
            ...
            <dl className={`${SPACING.subsection} mt-6`}>
              <div>
                ...
                <dd className="text-muted-foreground ml-0 mt-2">
                  Widely-supported XML feed format with excellent compatibility
                  across all feed readers. Includes rich metadata and engagement
                  stats.
                </dd>
              </div>
              <div>
                ...
                <dd className="text-muted-foreground ml-0 mt-2">
                  Modern XML feed format with enhanced features and strict
                  validation. Excellent reader support.
                </dd>
              </div>
              <div>
                <dt className={TYPOGRAPHY.h3.standard}>JSON Feed</dt>
                <dd className="text-muted-foreground ml-0 mt-2">
                  Modern JSON-based format that&apos;s easier to parse for
