...
# Enhanced validation for code generated by GitHub Copilot or other AI models
...
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}🔍 Enhanced Validation (GitHub Copilot + AI Models)${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
if [[ "$FLEXIBLE_ONLY" == "false" ]]; then
  ...
  if [[ "$TOKEN_VIOLATIONS" -eq 0 ]]; then
    echo -e "${GREEN}✅ 0 violations${NC}"
  else
    echo -e "${RED}❌ $TOKEN_VIOLATIONS violations${NC}"
  ...
  if [[ "$COMPLIANCE_INT" -ge 90 ]]; then
    echo -e "${GREEN}✅ ${COMPLIANCE}% (target: ≥90%)${NC}"
  else
    echo -e "${RED}❌ ${COMPLIANCE}% (target: ≥90%)${NC}"
  ...
  if [[ "$DEEP_IMPORTS" -eq 0 ]]; then
    ...
    echo -e "${RED}❌ $DEEP_IMPORTS violations${NC}"
  ...
  if [[ "$TEST_DATA_IN_SRC" -eq 0 ]]; then
    ...
    echo -e "${RED}❌ $TEST_DATA_IN_SRC violations${NC}"
  ...
  EMOJI_IN_UI=$(grep -r "🎯\|🚀\|✨\|💡\|🔥\|📝\|✅\|❌\|⚠️" src app --include="*.tsx" --include="*.md" --exclude="*.test.*" 2>/dev/null | wc -l || echo "0")
  EMOJI_IN_UI=$(echo "$EMOJI_IN_UI" | tr -d ' ')
  
  if [[ "$EMOJI_IN_UI" -eq 0 ]]; then
    echo -e "${GREEN}✅ 0 violations${NC}"
  else
    echo -e "${RED}❌ $EMOJI_IN_UI violations${NC}"
    STRICT_VIOLATIONS=$((STRICT_VIOLATIONS + EMOJI_IN_UI))
    
    if [[ "$VERBOSE" == "true" ]]; then
      echo ""
      grep -rn "🎯\|🚀\|✨\|💡\|🔥\|📝\|✅\|❌\|⚠️" src app --include="*.tsx" --include="*.md" --exclude="*.test.*" 2>/dev/null || true
      echo ""
    fi
  fi
...
if [[ "$STRICT_ONLY" == "false" ]]; then
  ...
  if [[ "$INNGEST_RATIO_INT" -ge 80 ]]; then
    echo -e "${GREEN}✅ ${INNGEST_RATIO}% Inngest usage (target: ≥80%)${NC}"
  else
    echo -e "${YELLOW}⚠️  ${INNGEST_RATIO}% Inngest usage (target: ≥80%)${NC}"
  ...
  if [[ "$TEST_PASS_RATE_INT" -ge 99 ]]; then
    echo -e "${GREEN}✅ ${TEST_PASS_RATE}% pass rate (target: ≥99%)${NC}"
  else
    echo -e "${YELLOW}⚠️  ${TEST_PASS_RATE}% pass rate (target: ≥99%)${NC}"
    FLEXIBLE_WARNINGS=$((FLEXIBLE_WARNINGS + 1))
    
    if [[ "$VERBOSE" == "true" ]]; then
      echo ""
      cat /tmp/test-output.txt | grep "FAIL\|✕" || true
      echo ""
    fi
  fi
...
if [[ "$STRICT_VIOLATIONS" -eq 0 ]] && [[ "$FLEXIBLE_WARNINGS" -eq 0 ]]; then
  ...
  echo -e "${YELLOW}⚠️  WARNINGS (Manual Review Required)${NC}"
  echo "   STRICT rules: ✅ Pass"
  ...
  echo -e "${RED}❌ VALIDATION FAILED (HARD BLOCK)${NC}"
  echo "   STRICT violations: $STRICT_VIOLATIONS"
  echo "   FLEXIBLE warnings: $FLEXIBLE_WARNINGS"
  echo ""
  echo "🔧 FIX STRICT VIOLATIONS BEFORE COMMITTING"
  echo ""
  echo "Quick fixes:"
