...
describe('BackToTop', () => {
  ...
  describe('Event Listeners', () => {
    ...
    it('uses passive scroll listener for performance', () => {
      ...
      const calls = addEventListenerSpy.mock.calls.filter((call) => call[0] === 'scroll');
  ...
  describe('Styling', () => {
    it('applies correct positioning classes with safe area and BottomNav clearance', async () => {
      render(<BackToTop />);
      ...
      await waitFor(() => {
        ...
        expect(container?.className).toContain('bottom-[76px]'); // Clears 64px BottomNav + 8px spacing + 4px margin
        expect(container?.className).toContain('pb-[env(safe-area-inset-bottom,0px)]'); // iOS safe area support
