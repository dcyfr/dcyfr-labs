 ...
 * ```
 */
export const getAnalyticsData = cache(async (days: number | null = null): Promise<AnalyticsData> => {
  try {
 ...
 * ```
 */
export const getDailyAnalyticsData = cache(async (days: number | null = null): Promise<DailyData[]> => {
  try {
    // Generate date range
