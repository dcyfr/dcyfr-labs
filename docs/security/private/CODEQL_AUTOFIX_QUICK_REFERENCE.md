...
- ⚠️ DOM XSS (often false positives)
- ⚠️ Complex path validation
- ⚠️ Hardcoded passwords (may be tests)

**Skipped:**

- ❌ Alerts below minimum severity
- ❌ Already dismissed/suppressed
...
```
┌─ CodeQL Scans (Daily 06:00 UTC)
│
├─ Autofix Analyzes (Daily 07:00 UTC)
│  ├─ Fetch alerts
│  ├─ Filter by severity (high/critical/etc)
│  └─ Identify fixable rules
│
├─ For Each Fixable Alert (Parallel)
│  ├─ Create branch: security/codeql-{number}-{rule}
│  ├─ Request Copilot fix
│  ├─ Validate (TypeScript, ESLint, tests)
...
```
CRITICAL   └─ Auto-fixed (if fixable)
HIGH       └─ Auto-fixed (if fixable) ← Default
MEDIUM     └─ Requires explicit trigger
LOW        └─ Requires explicit trigger
WARNING    └─ Requires explicit trigger
NOTE       └─ Requires explicit trigger
```
