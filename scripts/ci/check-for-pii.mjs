...
function filterEmailMatches(matches, allowlist) {
  return matches.filter((m) => {
    ...
    if (parts.length === 2) {
      ...
      if ((allowlist.emailDomains || []).some((d) => domain.includes(d)))
        return false;
    }
    return true;
  });
...
function filterPhoneMatches(matches) {
  return matches.filter((m) => {
    ...
    if (
      /^(1234567890|0123456789|0000000000|1111111111|2222222222|9999999999)$/.test(
        digits
      )
    )
      return false;
...
function filterPrivateKeyMatches(matches, file, content, allowlist) {
  return matches.filter((m) => {
    ...
    const isPlaceholder =
      /EXAMPLE|REDACTED|REPLACE|REPLACE_ME|DUMMY|YOUR_PRIVATE_KEY|PLACEHOLDER|\[REDACTED\]|YOUR_PRIVATE_KEY_EMAIL|YOUR KEY HERE|INSERT.*KEY/gi.test(
        surrounding
      );
    if (isInAllowlist(file, allowlist.privateKeyPaths)) {
      if (!isPlaceholder)
        console.warn(
          `⚠️  Warning: Private key found in allowlisted doc ${file} without clear redaction markers`
        );
...
function filterJwtMatches(matches, content) {
  return matches.filter((m) => {
    const surrounding = content.slice(
      Math.max(0, content.indexOf(m) - 50),
      content.indexOf(m) + m.length + 50
    );
    if (/EXAMPLE|PLACEHOLDER|REDACTED|FAKE|MOCK|TEST|DUMMY/i.test(surrounding))
      return false;
    if (/\/\/|--|\s*#/.test(surrounding.substring(0, surrounding.indexOf(m))))
      return false;
    return true;
  });
}
...
function filterOauthMatches(matches, isContentFile, content) {
  return matches.filter((m) => {
    ...
    if (
      /EXAMPLE|PLACEHOLDER|REDACTED|FAKE|MOCK|TEST|DUMMY|example\.com|localhost|placeholder/i.test(
        surrounding
      )
    )
      return false;
...
function filterDatabaseMatches(matches, content) {
  return matches.filter((m) => {
    ...
    if (
      /example|placeholder|localhost|test|127\.0\.0\.1|docker|EXAMPLE|PLACEHOLDER|TEST|DUMMY/i.test(
        m
      )
    )
      return false;
    if (/EXAMPLE|PLACEHOLDER|REDACTED|FAKE|MOCK|TEST/i.test(surrounding))
      return false;
...
function filterBusinessIdMatches(
  matches,
  file,
  isContentFile,
  content,
  ...
  return matches.filter((m) => {
    if (
      /EXAMPLE|PLACEHOLDER|REDACTED|FAKE|MOCK|TEST|DUMMY|example\.com|placeholder/i.test(
        m
      )
    )
      return false;
...
function applyMatchFilters(
  ...
  if (t.name === "private_key")
    return filterPrivateKeyMatches(matches, file, content, allowlist);
  if (t.name === "jwt_token") return filterJwtMatches(matches, content);
  if (t.name === "oauth_token")
    return filterOauthMatches(matches, isContentFile, content);
  if (t.name === "database_connection")
    return filterDatabaseMatches(matches, content);
  if (t.name === "generic_api_key")
    return filterApiKeyMatches(matches, content);
  if (
    [
      "customer_id",
      "order_number",
    ...
    return filterBusinessIdMatches(
      matches,
      file,
      isContentFile,
      content,
...
function buildResultEntry(t, file, matches, allowlist) {
  ...
  if (
    t.name === "private_key" &&
    isInAllowlist(file, allowlist.privateKeyPaths)
  ) {
    const reason = getAllowlistReason(file);
    entry.message = `${entry.message} (allowlisted path${
      reason ? `: ${reason}` : ""
    })`;
...
function scanContent(file, content, results, isPrScan, isContentFile) {
  ...
  const tests = [
    ...
    },
    {
      ...
      regex:
        /console\.(log|error|warn|info|debug)\([^)]*\$\{[^}]*\b(clientIp|ipAddress|ip|realIp|forwardedIp|remoteIp)\b/gi,
      message:
        "potential IP address logging in console statement (PII under GDPR/CCPA)",
    ...
    },
    {
      ...
      // eyJ is base64 for {"
      regex:
        /(?:Bearer|JWT)\s+eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/gi,
    ...
    },
    {
      ...
      regex:
        /(?:access_token|refresh_token|oauth_token|api_token|token)\s*[=:]\s*["']?([a-zA-Z0-9._\-]{20,}|[a-f0-9]{32,})["']?/gi,
    ...
    },
    {
      ...
      regex:
        /(postgresql|mysql|mongodb\+srv|mongodb|redis|mariadb|cockroachdb):\/\/([a-zA-Z0-9._\-%]+:[a-zA-Z0-9._\-%$@!]+@|:([a-zA-Z0-9._\-%$@!]+)@)[a-zA-Z0-9._\-:]+\/[a-zA-Z0-9._\-]*/gi,
    ...
    },
    {
      ...
      regex:
        /(?:api_key|apiKey|API_KEY|api-key)\s*[=:]\s*["']?([a-zA-Z0-9._\-]{20,}|[a-f0-9]{32,}|sk-[a-zA-Z0-9]{20,})["']?/gi,
    ...
    // ============================================================================
    {
      ...
      regex:
        /(?:customer[-_]?id|cust[-_]?(?:id|no)|customer[-_]?number)\s*[=:]\s*["']?([A-Z\d-]{4,20}|\d{6,})["']?/gi,
    ...
    },
    {
      ...
      regex:
        /(?:order[-_]?(?:id|no|number)|order)\s*[=:]\s*["']?([A-Z]{2,4}-\d{4}-\d{4,}|[A-Z\d]{4,})["']?/gi,
    ...
    },
    {
      ...
      regex:
        /(?:transaction[-_]?(?:id|no)|txn[-_]?(?:id|no)|trans[-_]?(?:id|no))\s*[=:]\s*["']?([A-Z\d-]{4,20}|\d{6,})["']?/gi,
    ...
    },
    {
      ...
      regex:
        /(?:account[-_]?(?:id|no|number)|acct(?:[-_]?(?:no|id))?)\s*[=:]\s*["']?([A-Z0-9]{4,20})["']?/gi,
  ...
  if (isPrScan) {
    tests.push({
      name: "proprietary",
      regex:
        /\b(PROPRIETARY|CONFIDENTIAL|PROPRIETARY INFORMATION|TRADE SECRET|DO NOT SHARE)\b/gi,
  ...
  for (const t of tests) {
    ...
    if (t.name === "proprietary") {
      ...
      if (
        isInAllowlist(file, allowlist.proprietaryPaths) ||
        isInAllowlist(file, allowlist.piPaths)
      ) {
        const reason = getAllowlistReason(file);
        if (reason)
          console.log(`Skipping proprietary check for ${file}: ${reason}`);
        continue;
      }
    }
    ...
    const isPiiAllowed =
      isInAllowlist(file, allowlist.piiPaths) || isPathAllowed(file);
    const isPiAllowed =
      isInAllowlist(file, allowlist.piPaths) ||
      isInAllowlist(file, allowlist.proprietaryPaths);
    if (
      classification === "PII" &&
      isPiiAllowed &&
      !["private_key", "aws_key", "proprietary"].includes(t.name)
    )
      continue;
    if (
      classification === "PI" &&
      isPiAllowed &&
      !["private_key", "aws_key"].includes(t.name)
    )
      continue;
...
async function main() {
  ...
  if (results.length > 0) {
    ...
    for (const r of results) {
      ...
      console.error(
        `  Examples: ${r.matches.map((m) => String(m)).join(", ")}`
      );
      console.error("");
    }
    console.error(
      "If this content is expected, move files to a secure location or add a rationale."
    );
    process.exit(1);
  }
