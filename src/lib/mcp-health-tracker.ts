 ...
 */
export async function getAllUptimeMetrics(days: number = 7): Promise<Record<string, McpUptimeMetrics>> {
...
async function checkAndAlert(report: McpHealthReport): Promise<void> {
  const downCritical = report.servers.filter(s => 
    CRITICAL_MCPS.includes(s.name) && s.status === 'down'
  );
  ...
  const degraded = report.servers.filter(s => 
    s.status === 'ok' && s.responseTimeMs > 5000
  );
